...
public class CopiaImuno implements CopiaInterface {

	@Override
	public void copia(Unidade unidade, boolean substituirArquivosNoDestino) {
		System.out.println(" Iniciando cópia dos arquivos da IMUNO: "
		...
		if (pasta.exists()) {
			...
			for (File file : listaDeArquivos) {
				...
				destino = new File("//" + unidade.getIp() + "/e/INT_IMU/ENV/"
						+ origem.getName());
		...
		System.out.println(" Finalizando cópia dos arquivos da IMUNO: "
				+ Principal.formatador.format(new Date()) + "\n");
	}
