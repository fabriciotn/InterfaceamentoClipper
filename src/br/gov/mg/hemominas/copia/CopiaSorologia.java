...
public class CopiaSorologia implements CopiaInterface {

	@Override
	public void copia(Unidade unidade, boolean substituirArquivosNoDestino) {
		System.out.println(" Iniciando cópia dos arquivos da SOROLOGIA: "
		...
		if (pasta.exists()) {
			...
			for (File file : listaDeArquivos) {
				...
				destino = new File("//" + unidade.getIp() + "/e/INT_SOR/ENV/"
						+ origem.getName());
		...
		System.out.println(" Finalizando cópia dos arquivos da SOROLOGIA: "
				+ Principal.formatador.format(new Date()) + "\n");
	}
